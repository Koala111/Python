    ...
    def login_host(self,host_ip,username,password,su,su_pwd):
        try:
            try:
                ...
                # self.telnet.read_until(b"$", 10)  # 判断输入账户后，返回值为Password:，就进行下一步输入
...
if __name__ == '__main__':
    ...
    if telnet_client.login_host(host_ip,username,password,su,su_pwd):
        free = str(telnet_client.import_content("cat /proc/`pidof gdecms`/status | grep VmSize | grep -v grep | awk '{print $2}'")).split()[-2]
